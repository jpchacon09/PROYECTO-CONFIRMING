import { NextRequest, NextResponse } from 'next/server'
import { supabaseAdmin } from '@/lib/supabase/admin'
import { ApiResponse, EstadoEmpresa } from '@/lib/types'

const ESTADOS_VALIDOS: EstadoEmpresa[] = [
  'pendiente',
  'en_revision',
  'documentos_incompletos',
  'aprobado',
  'rechazado'
]

export async function PATCH(
  req: NextRequest,
  { params }: { params: { id: string } }
) {
  try {
    // Validar autenticación
    const authHeader = req.headers.get('authorization')
    if (!authHeader) {
      return NextResponse.json<ApiResponse<null>>({
        success: false,
        error: {
          code: 'UNAUTHENTICATED',
          message: 'Token de autenticación requerido'
        }
      }, { status: 401 })
    }

    // Verificar usuario y que sea admin
    const { data: { user }, error: userError } = await supabaseAdmin.auth.getUser(
      authHeader.replace('Bearer ', '')
    )

    if (userError || !user) {
      return NextResponse.json<ApiResponse<null>>({
        success: false,
        error: {
          code: 'UNAUTHENTICATED',
          message: 'Token inválido o expirado'
        }
      }, { status: 401 })
    }

    // Verificar rol admin
    const { data: usuario } = await supabaseAdmin
      .from('usuarios')
      .select('rol')
      .eq('id', user.id)
      .single()

    if (usuario?.rol !== 'admin') {
      return NextResponse.json<ApiResponse<null>>({
        success: false,
        error: {
          code: 'ADMIN_REQUIRED',
          message: 'Solo administradores pueden cambiar el estado'
        }
      }, { status: 403 })
    }

    // Parsear body
    const body = await req.json()
    const { nuevo_estado, motivo } = body

    // Validar estado
    if (!nuevo_estado || !ESTADOS_VALIDOS.includes(nuevo_estado)) {
      return NextResponse.json<ApiResponse<null>>({
        success: false,
        error: {
          code: 'INVALID_ESTADO',
          message: `Estado inválido. Valores permitidos: ${ESTADOS_VALIDOS.join(', ')}`
        }
      }, { status: 400 })
    }

    // Obtener empresa actual
    const { data: empresa, error: empresaError } = await supabaseAdmin
      .from('empresas_pagadoras')
      .select('*')
      .eq('id', params.id)
      .single()

    if (empresaError || !empresa) {
      return NextResponse.json<ApiResponse<null>>({
        success: false,
        error: {
          code: 'EMPRESA_NOT_FOUND',
          message: 'La empresa no existe'
        }
      }, { status: 404 })
    }

    // Actualizar estado
    const { data: empresaActualizada, error: updateError } = await supabaseAdmin
      .from('empresas_pagadoras')
      .update({
        estado: nuevo_estado,
        aprobado_por: nuevo_estado === 'aprobado' ? user.id : null,
        fecha_aprobacion: nuevo_estado === 'aprobado' ? new Date().toISOString() : null
      })
      .eq('id', params.id)
      .select()
      .single()

    if (updateError) {
      console.error('Error al actualizar estado:', updateError)
      return NextResponse.json<ApiResponse<null>>({
        success: false,
        error: {
          code: 'DATABASE_ERROR',
          message: 'Error al actualizar el estado de la empresa'
        }
      }, { status: 500 })
    }

    // El trigger de Supabase se encarga de crear el registro en historial_estados
    // y la notificación en notificaciones_enviadas

    return NextResponse.json<ApiResponse<any>>({
      success: true,
      data: {
        empresa_id: params.id,
        estado_anterior: empresa.estado,
        estado_nuevo: nuevo_estado,
        cambiado_por: user.id,
        fecha_cambio: new Date().toISOString()
      }
    })
  } catch (error) {
    console.error('Error en PATCH /api/admin/empresas/[id]/estado:', error)
    return NextResponse.json<ApiResponse<null>>({
      success: false,
      error: {
        code: 'INTERNAL_ERROR',
        message: 'Error interno del servidor'
      }
    }, { status: 500 })
  }
}
